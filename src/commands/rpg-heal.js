let handler = async (m, { conn, usedPrefix, command }) => {
    ...
    if (!user) return conn.reply(m.chat, `ꕤ El usuario no se encuentra en la base de Datos.`, m)
    if (user.health >= 100) return conn.reply(m.chat, `ꕤ Tu salud ya está al máximo.`, m)
    if (user.coin <= 0) return conn.reply(m.chat, `ꕤ No tienes ${userCurrency} suficientes para curarte.`, m)
