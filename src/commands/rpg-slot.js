...
const handler = async (m, { args, usedPrefix, command, conn }) => {
    ...
    if (Date.now() - users.lastslot < 10000) {
        ...
        return m.reply(`ꕤ Debes esperar *${formatTime(restante)}* para usar *${usedPrefix + command}* nuevamente.`)
    }
    if (apuesta < 100) return m.reply(`ꕤ El mínimo para apostar es de 100 *${userCurrency}*.`)
    if (users.coin < apuesta) return m.reply(`ꕤ Tus *${userCurrency}* no son suficientes para apostar esa cantidad.`)
    const emojis = ['✾', '❃', '❁']
...
────────`
    ...
    let resultado
    if (x[0] === y[0] && y[0] === z[0]) {
        resultado = `❀ Ganaste! *$${(apuesta * 2).toLocaleString()} ${userCurrency}*.`
        users.coin += apuesta
    } else if (x[0] === y[0] || x[0] === z[0] || y[0] === z[0]) {
        resultado = `❀ Casi lo logras. *Toma $10 ${userCurrency}* por intentarlo.`
        users.coin += 10
    } else {
        resultado = `❀ Perdiste *$${apuesta.toLocaleString()} ${userCurrency}*.`
        users.coin -= apuesta
    }
