var handler = async (m, { conn, usedPrefix }) => {
    if (!db.data.chats[m.chat].economy && m.isGroup) return m.reply(`ꕤ Los comandos de *Economía* están desactivados en este grupo.\n\nUn *administrador* puede activarlos con el comando:\n» *${usedPrefix}economy on*`)
    ...
    if (now < user.lastweekly) {
        ...
        return conn.reply(m.chat, `ꕤ Ya has reclamado tu recompensa semanal.\n> Puedes reclamarlo de nuevo en *${wait}*`, m, (global.rcanalr || {}))
    }
    ...
    conn.reply(m.chat, `「❁」 Has reclamado tu recompensa semanal de *$${coins.toLocaleString()} ${userCurrency}* (Semana *${user.weeklyStreak}*)\n${msg}`, m, (global.rcanalr || {}))
}
