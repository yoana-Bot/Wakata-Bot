...
let handler = async (m, { conn, args, usedPrefix, command }) => {
    ...
    if (user.lastApuesta && ahora - user.lastApuesta < tiempoEspera) {
        ...
        return conn.reply(m.chat, `ꕤ Debes esperar *${tiempoRestante}* para usar *${usedPrefix + command}* nuevamente.`, m)
    }
    ...
    if (args.length < 1) {
        return conn.reply(m.chat, `ꕤ Ingresa la cantidad de *${userCurrency}* que deseas aportar contra *${botname}*\n> Ejemplo: *${usedPrefix + command} 100*`, m)
    }
    ...
    if (user.coin >= count) {
        ...
        if (Aku > Kamu) {
            resultado = `> ${userName}, *Perdiste $${formatNumber(count)} ${userCurrency}*.`
        } else if (Aku < Kamu) {
            ganancia = count * 2
            user.coin += ganancia
            resultado = `> ${userName}, *Ganaste $${formatNumber(ganancia)} ${userCurrency}*.`
        } else {
            ganancia = count
            user.coin += ganancia
            resultado = `> ${userName}, *Ganaste $${formatNumber(ganancia)} ${userCurrency}*.`
        ...
        const replyMsg = `ꕤ \`Veamos qué números tienen!\`\n\n➠ *${botname}* : ${Aku}\n➠ *${userName}* : ${Kamu}\n\n` +
        resultado
        
        conn.reply(m.chat, replyMsg.trim(), m)
    } else {
        conn.reply(m.chat, `ꕤ No tienes *$${formatNumber(count)} ${userCurrency}* para apostar!`, m)
    }
}
