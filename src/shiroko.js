    ...
    try {
        ...
        try {
            ...
            if (!user) {
                global.db.data.users[m.sender] = {
                    name: m.name,
                    exp: 0, coin: 0, bank: 0, level: 0, health: 100, genre: "", birth: "", marry: "",
                    description: "", packstickers: null, premium: false, premiumTime: 0,
            ...
            if (!chat) {
                global.db.data.chats[m.chat] = {
                    isBanned: false, isMute: false, welcome: global.modes.welcome,
                    sWelcome: "", sBye: "", detect: global.modes.detect, primaryBot: null,
                    modoadmin: global.modes.modoadmin, antiLink: global.modes.antilink,
                    nsfw: global.modes.nsfw, economy: global.modes.economy, gacha: global.modes.gacha
                }
            }
            ...
            if (!settings) {
                global.db.data.settings[this.user.jid] = {
                    self: global.modes.self, jadibotmd: global.modes.jadibotmd,
                    autoread: global.modes.autoread, autoreaction: global.modes.autoreaction,
        ...
        if (m.isGroup) {
            ...
            if (cached && (now - cached.timestamp) < 30000) {
                ...
                groupMetadataCache.set(cacheKey, { metadata: groupMetadata, participants, timestamp: now })
            }
        ...
        for (const name in global.plugins) {
            ...
            if (typeof plugin.all === "function") {
                try { await plugin.all.call(this, m, { chatUpdate, __dirname: ___dirname, __filename, user, chat, settings }) } catch (err) { console.error(err) }
            }
            ...
            if ((usedPrefix = (match[0] || "")[0])) {
                ...
                if (chat.primaryBot && chat.primaryBot !== this.user.jid) {
                    ...
                    if (primaryBotConn || chat.primaryBot === global.conn.user.jid) return
                ...
                if (chat) {
                    ...
                    if (m.text && user.banned && !isROwner) {
                        await m.reply(global.msg.mensaje.replace('${bannedReason}', user.bannedReason))
                        return
                    }
                }
